...
const RegistrationService = {
  registerUserAndCompany: async (userData, companyData, paySlipData) => {
    try {
      // Validate user data
      ...
      const { error: paySlipError } =
        Validation.validatePaySlipData(paySlipData);
      if (paySlipError) {
      ...
      const paySlipInfo =
        await CompanyPaySlipRepository.createCompanyPaySlipInfo(paySlipData);
